...
contextBridge.exposeInMainWorld('lockOn', {
  ...
  service: {
    ...
    updateSchedule: (orderId: string, payload: unknown) => ipcRenderer.invoke('service:updateSchedule', orderId, payload),
    reorderTechnicianDay: (date: string, orderIds: string[]) => ipcRenderer.invoke('service:reorderTechnicianDay', date, orderIds),
    saveWarranty: (orderId: string, months: number) => ipcRenderer.invoke('service:saveWarranty', orderId, months),

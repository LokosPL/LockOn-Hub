...
const route = async (request) => {
  ...
  if(method==='GET'&&url.pathname==='/support/tickets'){
    ...
    const tickets=[];
    for(const row of rows){
      const messages=(await q("SELECT id,sender_user_id,sender_kind,body,metadata,created_at FROM support_messages WHERE conversation_id=$1 AND (metadata->>'target'='CONSULTANT' OR sender_kind='SUPPORT' OR (sender_kind='USER' AND $2::timestamptz IS NOT NULL AND created_at >= $2::timestamptz)) ORDER BY created_at ASC LIMIT 200",[row.id,row.consultant_joined_at||row.taken_at||null])).rows;

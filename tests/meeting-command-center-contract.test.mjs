...
test('1.0.5.22 keeps the live room mounted while navigating through ServiceOS', () => {
...
test('1.0.5.22 browser remains interactive beside a live meeting', () => {
  const app = read('src/App.tsx');
...
test('1.0.5.22 exposes a persistent host-only broadcast overlay for screen and window sharing', () => {
...
test('1.0.5.22 theater mode maximizes the stage and keeps collaboration visible', () => {
  const room = read('src/components/MeetingRoom.tsx');
  const styles = read('src/styles.css');
...
test('1.0.5.22 removes duplicate local stream tile and keeps only remote-share thumbnails', () => {
  const room = read('src/components/MeetingRoom.tsx');
...
test('1.0.5.22 keeps stop-sharing only in the active share strip', () => {
  const room = read('src/components/MeetingRoom.tsx');

  assert.match(room, /meeting-local-share-actions/);
  assert.match(room, /className="danger" onClick=\{\(\)=>void stopScreenShare\(\)\}/);
  assert.doesNotMatch(room, /screenEnabled&&<button className="meeting-control"[^\n]+stopScreenShare/);
});

test('1.0.5.22 keeps clean self-share stage and explicit local preview', () => {
  const room = read('src/components/MeetingRoom.tsx');
...
test('1.0.5.22 retains high-resolution screen share and low-latency speech', () => {
  const room = read('src/components/MeetingRoom.tsx');
  const electron = read('electron/main.ts');
...
test('1.0.5.23 product and updater versions stay aligned', () => {
  assert.equal(read('RELEASE_VERSION').trim(), '1.0.5.23');
  assert.equal(read('BUILD_VERSION').trim(), '1.5.23');
  assert.equal(JSON.parse(read('package.json')).version, '1.5.23');
  assert.match(read('electron/appConfig.ts'), /productVersion: '1\.0\.5\.23'/);
});
